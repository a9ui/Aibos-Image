...
public partial class MainWindow : Window
{
    ...
    private long _queuedGalleryFocusCompletionGeneration = -1;
    private DispatcherOperation? _galleryFocusCompletionOperation;
    private string? _pendingGalleryFocusPath;
    private int _pendingGalleryFocusIndex = -1;
    private bool _pendingGalleryFocusGrid;
    private int _pendingGalleryFocusRetryCount;
    private const int MaxPendingGalleryFocusRetryCount = 3;
    private const int CatalogResetSelectionScanReleaseThreshold = 24_000;
    ...
    private double _modalEdgeNavigationPercent = ModalEdgeNavigationDefaultPercent;
    private bool _syncingModalEdgeNavigationPercent;
    ...
    private DispatcherTimer? _statusToastDismissTimer;
    ...
    public MainWindow()
    {
        ...
        _statusToastDismissTimer.Tick += (_, _) =>
        {
        ...
        _modalTransformQualityTimer = new DispatcherTimer(DispatcherPriority.Render)
        {
            Interval = TimeSpan.FromMilliseconds(ModalTransformQualitySettleMilliseconds),
        };
        _modalTransformQualityTimer.Tick += (_, _) => EndModalTransformInteraction();
        ...
        Closed += (_, _) =>
        {
            ...
            CardsList.CancelAutomationRealization();
            RowsList.CancelAutomationRealization();
        ...
        int lastRealizedIndex)
    {
        ...
        SetProtectedResidentThumbnailRanges(firstVisible, lastVisible, firstRealized, lastRealized);
    ...
    private void ScheduleListThumbnailViewport()
    {
        ...
        SetProtectedResidentThumbnails(protectedTiles);
    ...
    private readonly record struct CatalogProjectionApplyMetrics(
        ...
        long PreResetMs = 0,
        long PreResetSelectionReleaseMs = 0,
        bool PreResetSelectionCleared = false,
        int PreResetSelectionReleasedCount = 0,
        long ResetMs = 0,
    ...
    private bool ShowAlbumLibrary()
    {
        ...
        var window = new AlbumLibraryWindow(
            ...
            ActivateAlbumSourceAsync,
            RefreshActiveAlbumSourceAsync);
        window.ShowDialog();
        return true;
    }
    ...
    {
        // Kept for deterministic UI smoke helpers only. Production paths use
        // the cancelable asynchronous source activation below.
        Interlocked.Increment(ref _activeAlbumAsyncGeneration);
        Interlocked.Exchange(ref _activeAlbumAvailabilityCts, null)?.Cancel();
        if (album is null)
        {
            ApplyActiveAlbumSource(null, 0, 0);
            return;
        }
        ...
        int missing = 0;
        int outside = 0;
        foreach (AlbumMemberEntry member in album.Members)
        {
            bool exists = File.Exists(member.ImagePath);
            if (!exists)
                missing++;
            else if (!catalogPaths.Contains(member.ImagePath))
                outside++;
        }
        ApplyActiveAlbumSource(album, outside, missing);
    }

    private async Task ActivateAlbumSourceAsync(AlbumEntry? album)
    {
        (int generation, CancellationTokenSource cancellation) = BeginActiveAlbumAvailabilityRequest();
        try
        {
            await ApplyActiveAlbumSourceAsync(album, generation, cancellation);
        }
        finally
        {
            CompleteActiveAlbumAvailabilityRequest(cancellation);
        }
    }

    private async Task ApplyActiveAlbumSourceAsync(
        AlbumEntry? album,
        int generation,
        CancellationTokenSource cancellation)
    {
        cancellation.Token.ThrowIfCancellationRequested();
        if (generation != Volatile.Read(ref _activeAlbumAsyncGeneration))
            return;
        if (album is null)
        {
            ApplyActiveAlbumSource(null, 0, 0);
        ...
        var catalogPaths = _allTiles.Where(static tile => tile.IsRealFile)
            .Select(static tile => tile.Path)
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        IReadOnlyList<string> memberPaths = album.Members.Select(static member => member.ImagePath).ToArray();
        // Publish the source immediately; filesystem availability is only
        // supplemental status and must never hold up Album navigation.
        ApplyActiveAlbumSource(album, 0, 0);
        Interlocked.Increment(ref _activeAlbumAvailabilityPendingCount);
        try
        {
            await _activeAlbumAvailabilityGate.WaitAsync(cancellation.Token);
            int active = Interlocked.Increment(ref _activeAlbumAvailabilityActiveCount);
            UpdateMaximumCounter(ref _activeAlbumAvailabilityMaxConcurrentCount, active);
            Interlocked.Increment(ref _activeAlbumAvailabilityStartedCount);
            (int Outside, int Missing) availability;
            try
            {
                availability = await Task.Run(() =>
                {
                    _lastActiveAlbumAvailabilityOffDispatcher = !Dispatcher.CheckAccess();
                    int missing = 0;
                    int outside = 0;
                    foreach (string path in memberPaths)
                    {
                        cancellation.Token.ThrowIfCancellationRequested();
                        int delay = Volatile.Read(ref _activeAlbumAvailabilityDelayForSmokeMs);
                        if (delay > 0 && cancellation.Token.WaitHandle.WaitOne(delay))
                            cancellation.Token.ThrowIfCancellationRequested();
                        bool exists = File.Exists(path);
                        if (!exists)
                            missing++;
                        else if (!catalogPaths.Contains(path))
                            outside++;
                    }
            ...
            {
                Interlocked.Decrement(ref _activeAlbumAvailabilityActiveCount);
                _activeAlbumAvailabilityGate.Release();
            }

            if (cancellation.IsCancellationRequested
                || generation != Volatile.Read(ref _activeAlbumAsyncGeneration)
                || !string.Equals(_activeAlbumId, album.Id, StringComparison.Ordinal))
                return;
            _activeAlbumOutsideCount = availability.Outside;
            _activeAlbumMissingCount = availability.Missing;
            Interlocked.Increment(ref _activeAlbumAvailabilityApplyCount);
            UpdateFolderStats();
        }
        catch (OperationCanceledException) when (cancellation.IsCancellationRequested)
        {
            Interlocked.Increment(ref _activeAlbumAvailabilityCanceledCount);
        }
        finally
        {
            Interlocked.Decrement(ref _activeAlbumAvailabilityPendingCount);
        }
    }

    private (int Generation, CancellationTokenSource Cancellation) BeginActiveAlbumAvailabilityRequest()
    {
        int generation = Interlocked.Increment(ref _activeAlbumAsyncGeneration);
        var cancellation = new CancellationTokenSource();
        CancellationTokenSource? previous = Interlocked.Exchange(ref _activeAlbumAvailabilityCts, cancellation);
        previous?.Cancel();
        return (generation, cancellation);
    }

    private void CompleteActiveAlbumAvailabilityRequest(CancellationTokenSource cancellation)
    {
        Interlocked.CompareExchange(ref _activeAlbumAvailabilityCts, null, cancellation);
        cancellation.Dispose();
    }

    private static void UpdateMaximumCounter(ref int location, int value)
    {
    ...
    private void ApplyActiveAlbumSource(AlbumEntry? album, int outsideCount, int missingCount)
    {
        Interlocked.Increment(ref _activeAlbumSourceApplyCount);
        _activeAlbumMemberPaths.Clear();
        _activeAlbumMemberOrder.Clear();
        _activeAlbumId = album?.Id;
        _activeAlbumName = album?.Name;
        _activeAlbumMemberCount = album?.Members.Count ?? 0;
        _activeAlbumOutsideCount = outsideCount;
        _activeAlbumMissingCount = missingCount;
    ...
    private async Task RefreshActiveAlbumSourceAsync()
    {
        string? activeAlbumId = _activeAlbumId;
        if (activeAlbumId is null)
            return;
        (int generation, CancellationTokenSource cancellation) = BeginActiveAlbumAvailabilityRequest();
        try
        {
            AlbumReadResult read = await Task.Run(() => AlbumStore.Read(AlbumStore.ResolvePath()));
            cancellation.Token.ThrowIfCancellationRequested();
            if (generation != Volatile.Read(ref _activeAlbumAsyncGeneration)
                || !string.Equals(activeAlbumId, _activeAlbumId, StringComparison.Ordinal))
                return;
            AlbumEntry? active = read.Supported
                ? read.Document?.Albums.FirstOrDefault(album => album.Id == activeAlbumId)
                : null;
            await ApplyActiveAlbumSourceAsync(active, generation, cancellation);
        }
        catch (OperationCanceledException) when (cancellation.IsCancellationRequested)
        {
            Interlocked.Increment(ref _activeAlbumAvailabilityCanceledCount);
        ...
        {
            // Shared-state refresh is best effort. The already published source
            // remains usable and a later mutation/reload will retry it.
        }
        finally
        {
            CompleteActiveAlbumAvailabilityRequest(cancellation);
    ...
    private void QueueActiveAlbumSourceRefresh()
        => _ = RefreshActiveAlbumSourceAsync();
    ...
    {
        if (LandingFolderSetEmptyHint is not null)
        {
            LandingFolderSetEmptyHint.Visibility = _landingFolderSet.Count == 0
                ? Visibility.Visible
        ...
        if (LandingFolderStatusText is not null)
        {
            ...
            LandingFolderStatusText.Text = _landingFolderSet.Count == 0
                ? UiLanguageResources.Text("UiNoFoldersSelected")
                : UiLanguageResources.Format(
                    "UiUsableSelectedFoldersFormat",
                    existingCount,
                    _landingFolderSet.Count);
        }
    ...
    {
        long bytes = EstimateBitmapBytes(thumbnail);
        if (_residentThumbnailByteSizes.TryGetValue(tile.Path, out long previousBytes))
            _residentThumbnailBytes = Math.Max(0, _residentThumbnailBytes - previousBytes);
        _residentThumbnailByteSizes[tile.Path] = bytes;
        _residentThumbnailBytes += bytes;
        tile.Thumbnail = thumbnail;
        TouchResidentThumbnail(tile);
        TrimResidentThumbnails();
    }

    private void SetProtectedResidentThumbnailRanges(
        int firstVisibleIndex,
        int lastVisibleIndex,
        int firstRealizedIndex,
        int lastRealizedIndex)
    {
        ...
        AddProtectedResidentThumbnailRange(firstVisibleIndex, lastVisibleIndex);
        AddProtectedResidentThumbnailRange(firstRealizedIndex, lastRealizedIndex);
        FinishProtectedResidentThumbnailUpdate();
    }

    private void AddProtectedResidentThumbnailRange(int firstIndex, int lastIndex)
    {
        if (_tiles.Count == 0 || firstIndex < 0 || lastIndex < firstIndex)
            return;

        int first = Math.Clamp(firstIndex, 0, _tiles.Count - 1);
        int last = Math.Clamp(lastIndex, first, _tiles.Count - 1);
        for (int index = first; index <= last; index++)
        {
            Tile tile = _tiles[index];
            if (tile.IsRealFile)
                _protectedResidentThumbnailPaths.Add(tile.Path);
        }
    }

    private void SetProtectedResidentThumbnails(IEnumerable<Tile> tiles)
    {
        _protectedResidentThumbnailPaths.Clear();
    ...
    private void ClearProtectedResidentThumbnails()
    {
        if (_protectedResidentThumbnailPaths.Count == 0)
            return;
        _protectedResidentThumbnailPaths.Clear();
        TrimResidentThumbnails();
    }

    private void FinishProtectedResidentThumbnailUpdate()
    {
        if (SelectedTile() is { IsRealFile: true } selected)
            _protectedResidentThumbnailPaths.Add(selected.Path);
        TrimResidentThumbnails();
    }
    ...
    private void TrimResidentThumbnails()
    {
        ...
        while (_residentThumbnailBytes > effectiveBudget)
        {
            ...
            while (oldestUnprotected is not null
                && _protectedResidentThumbnailPaths.Contains(oldestUnprotected.Value.Path))
            {
                oldestUnprotected = oldestUnprotected.Next;
            }
            if (oldestUnprotected is null)
                break;
            EvictResidentThumbnail(oldestUnprotected);
        }
    ...
    private void EvictResidentThumbnail(LinkedListNode<Tile> node)
    {
        ...
        if (_protectedResidentThumbnailPaths.Contains(path))
        {
            _visibleThumbnailEvictionCount++;
            return;
        }

        _residentThumbnailLru.Remove(node);
        _residentThumbnailNodes.Remove(path);
        if (_residentThumbnailByteSizes.Remove(path, out long bytes))
            _residentThumbnailBytes = Math.Max(0, _residentThumbnailBytes - bytes);
        node.Value.Thumbnail = null;
    }

    private static long EstimateBitmapBytes(BitmapSource bitmap)
    {
        long width = Math.Max(1, bitmap.PixelWidth);
        long height = Math.Max(1, bitmap.PixelHeight);
        long bitsPerPixel = Math.Max(8, bitmap.Format.BitsPerPixel);
        long stride = checked(((width * bitsPerPixel) + 7) / 8);
        return checked(stride * height);
    }

    private static long ResolveResidentThumbnailBudgetBytes()
    {
        ...
        return Math.Clamp(
            candidate,
            MinResidentThumbnailBudgetBytes,
            MaxResidentThumbnailBudgetBytes);
    }
    ...
    private void TouchResidentThumbnail(Tile tile)
    {
        if (tile.Thumbnail is null)
        {
            ...
            if (_residentThumbnailByteSizes.Remove(tile.Path, out long staleBytes))
                _residentThumbnailBytes = Math.Max(0, _residentThumbnailBytes - staleBytes);
            return;
        }
    ...
    private void ClearResidentThumbnails()
    {
        ...
        _residentThumbnailByteSizes.Clear();
        _protectedResidentThumbnailPaths.Clear();
        _maxResidentThumbnailCount = 0;
        _residentThumbnailBytes = 0;
        _maxResidentThumbnailBytes = 0;
        _maxProtectedResidentThumbnailBytes = 0;
        _maxEffectiveResidentThumbnailBudgetBytes = _residentThumbnailBudgetBytes;
    ...
    /// <summary>
    /// Resolve the checkout that owns the shared browser/WPF stores. A linked
    /// Git worktree has its own project.toml but its .git file points back
    ...
    private static PersistenceLockLease? TryAcquirePersistenceLock(string targetPath, int timeoutMilliseconds)
    {
        ...
        while (true)
        {
            ...
            {
                // Legacy shared file locks are never pathname-reaped. The
                // object at lockPath may have been replaced after any stale
                // inspection, so deleting it here could remove a live Browser
                // owner's lock and admit concurrent writers. Fail closed until
                // an operator explicitly removes a proven crash orphan.
        ...
        IReadOnlyList<Tile> changedTiles)
    {
        ...
        foreach (Tile tile in changedTiles.Distinct())
        {
            if (MatchesFavoriteFilter(tile, favoritesOnly, unfavoriteOnly))
                continue;

            projectionChange = true;
            if (CardsList.TryResolveAutomationItem(tile, out int index))
        ...
        int selectedIndex = _primarySelectedIndex >= 0
            ? _primarySelectedIndex
            : Math.Max(CardsList.SelectedIndex, RowsList.SelectedIndex);
        if (!fallbackIndex.HasValue && selectedIndex >= 0)
            fallbackIndex = selectedIndex;

        // Keep every projection mutation on the scheduler path. It computes
    ...
    private static BitmapSource? LoadBitmap(string path, int decodePixelWidth)
    {
        try
        {
            ...
            if (decodePlan.Value.PixelWidth > 0)
                image.DecodePixelWidth = decodePlan.Value.PixelWidth;
            else if (decodePlan.Value.PixelHeight > 0)
    ...
    internal static ImageDimensions? ReadBitmapDimensionsForSmoke(string path)
        => TryReadBitmapSize(path, out int width, out int height)
            ? new ImageDimensions(width, height)
            : null;

    private static BitmapDecodePlan? BuildBitmapDecodePlanForDimensions(
        string extension,
        int requestedPixelWidth,
        int sourceWidth,
        int sourceHeight)
    ...
    private void GalleryList_PreviewKeyDown(object sender, KeyEventArgs e)
    {
        ...
        if (targetIndex != currentIndex)
        {
            ...
            CancelPendingGalleryFocus();
            // Keep the key event itself bounded. The viewport/focus jump is
            // queued below; preview/seen presentation follows at Background
            // priority so Input gets a turn between the distant container
            // realization and the right-panel layout.
            SetSelection(
                [target],
                target,
                targetIndex,
    ...
    private bool QueuePrimaryGalleryFocus(Tile tile, int index, bool grid)
    {
        ...
        _pendingGalleryFocusRetryCount = 0;
        if (grid)
        {
    ...
    private void QueuePendingGalleryFocusCompletion()
    {
        ...
        if (_pendingGalleryFocusIndex < 0
            || string.IsNullOrWhiteSpace(_pendingGalleryFocusPath)
            || _queuedGalleryFocusCompletionGeneration == generation
                && _galleryFocusCompletionOperation is { Status: DispatcherOperationStatus.Pending })
        {
            return;
        }

        if (_galleryFocusCompletionOperation is { Status: DispatcherOperationStatus.Pending } staleOperation)
            staleOperation.Abort();
        _queuedGalleryFocusCompletionGeneration = generation;
        _galleryFocusCompletionOperation = Dispatcher.BeginInvoke(() =>
        {
            if (_queuedGalleryFocusCompletionGeneration == generation)
            {
                _queuedGalleryFocusCompletionGeneration = -1;
                _galleryFocusCompletionOperation = null;
            }
    ...
    private void TryCompletePendingGalleryFocus()
    {
        ...
        if (index >= _tiles.Count)
            return;
        if (!string.Equals(_tiles[index].Path, path, StringComparison.OrdinalIgnoreCase))
            return;
        if (!string.Equals(_primarySelectedPath, path, StringComparison.OrdinalIgnoreCase))
            return;
        if (RealizedGalleryContainer(listBox, index) is not ListBoxItem container)
        {
            RetryPendingGalleryFocusCompletion(listBox, index);
            return;
        }
        if (container.DataContext is not Tile boundTile
    ...
    private void RetryPendingGalleryFocusCompletion(ListBox listBox, int index)
    {
        if (_pendingGalleryFocusRetryCount >= MaxPendingGalleryFocusRetryCount)
            return;
        _pendingGalleryFocusRetryCount++;
        if (ReferenceEquals(listBox, CardsList))
            _galleryVirtualizingPanel?.BringItemIntoView(index);
        else if (index >= 0 && index < _tiles.Count)
            RowsList.ScrollIntoView(_tiles[index]);
    ...
    {
        _galleryFocusRequestGeneration++;
        if (_galleryFocusCompletionOperation is { Status: DispatcherOperationStatus.Pending } operation)
            operation.Abort();
        _galleryFocusCompletionOperation = null;
        _queuedGalleryFocusCompletionGeneration = -1;
        _pendingGalleryFocusPath = null;
    ...
    private void QueuePrimarySelectionPresentation(Tile primary, int projectedIndex)
    {
        ...
        Dispatcher.BeginInvoke(() =>
        {
            ...
            // QueuePrimaryGalleryFocus already owns viewport realization for
            // keyboard navigation. Repeating ScrollIntoView here would merge
            // gallery and preview layout into one long dispatcher interval.
            ApplyPrimarySelection(primary, projectedIndex, ensureSelectionVisible: false);
        ...
        ReleaseWpfSelectionBeforeCatalogReset()
    {
        ...
        try
        {
            // WPF Selector.Reset locates any unresolved selected ItemInfo by
            // walking the new Items collection from index zero. Release only
            // WPF's bounded visual selection while its indices are still
            // current; Aibos keeps the canonical path selection and restores
            // the realized state after publication.
            if (CardsList.SelectedItems.Count > 0)
        ...
        return (
            watch.ElapsedMilliseconds,
            CardsList.SelectedItems.Count == 0
    ...
    private void ApplyPrimarySelection(
        Tile? primary,
        ...
        bool ensureSelectionVisible = true)
    {
        ...
        if (ensureSelectionVisible && CardsList.Visibility == Visibility.Visible)
        {
        ...
        if (ensureSelectionVisible && RowsList.Visibility == Visibility.Visible)
        {
    ...
    private bool ApplySearchHistoryWrite(SearchHistoryWriteResult result)
    {
        ...
        string message = result.Status == SearchHistoryWriteStatus.Protected
            ...
            : result.Status == SearchHistoryWriteStatus.Busy
                ? "Search history is busy in another Aibos window."
    ...
    private async Task ApplyCatalogProjectionAsync(CatalogProjectionRequest request)
    {
        ...
        request.Completion?.TrySetResult(outcome with
        {
            ...
            PreResetSelectionReleaseMs =
                applyMetrics.PreResetSelectionReleaseMs,
            PreResetSelectionCleared =
                applyMetrics.PreResetSelectionCleared,
            PreResetSelectionReleasedCount =
                applyMetrics.PreResetSelectionReleasedCount,
    ...
    {
        if (_initializing || _syncingUiLanguage)
            return;
        if (sender is FrameworkElement { Tag: string language })
            SetUiLanguage(language, persist: true);
    }

    private bool SetUiLanguage(string? language, bool persist)
    {
        string normalized = UiLanguageResources.Normalize(language);
        bool changed = !string.Equals(_uiLanguage, normalized, StringComparison.Ordinal);
        _uiLanguage = normalized;
        UiLanguageResources.Apply(normalized);

        _syncingUiLanguage = true;
        try
        {
            if (UiEnglishRadioButton is not null)
                UiEnglishRadioButton.IsChecked = normalized == UiLanguageResources.English;
            if (UiJapaneseRadioButton is not null)
                UiJapaneseRadioButton.IsChecked = normalized == UiLanguageResources.Japanese;
        }
        finally
        {
            _syncingUiLanguage = false;
        }

        if (UiLanguageStatusText is not null)
        {
            UiLanguageStatusText.Text = UiLanguageResources.Text(
                normalized == UiLanguageResources.Japanese
                    ? "UiLanguageAppliedJapanese"
                    : "UiLanguageAppliedEnglish");
        }

        SetModalEdgeNavigationPercent(_modalEdgeNavigationPercent, persist: false);
    ...
    {
        if (_initializing || _syncingModalEdgeNavigationPercent)
            return;
        SetModalEdgeNavigationPercent(e.NewValue, persist: true);
    }

    private bool SetModalEdgeNavigationPercent(double value, bool persist)
    {
        double normalized = Math.Clamp(
            Math.Round(double.IsFinite(value) ? value : ModalEdgeNavigationDefaultPercent),
            ModalEdgeNavigationMinPercent,
            ModalEdgeNavigationMaxPercent);
        bool changed = Math.Abs(normalized - _modalEdgeNavigationPercent) >= 0.01;
        _modalEdgeNavigationPercent = normalized;

        _syncingModalEdgeNavigationPercent = true;
        try
        {
            if (ModalEdgeNavigationSlider is not null
                && Math.Abs(ModalEdgeNavigationSlider.Value - normalized) >= 0.01)
            {
                ModalEdgeNavigationSlider.Value = normalized;
            }
            if (ModalEdgeNavigationValueText is not null)
                ModalEdgeNavigationValueText.Text = UiLanguageResources.Format("UiModalEdgeEachSideFormat", normalized.ToString("0"));
        }
        finally
        {
            _syncingModalEdgeNavigationPercent = false;
        }

        double center = Math.Max(0, 100 - (normalized * 2));
        ModalPreviousZoneColumn.Width = new GridLength(normalized, GridUnitType.Star);
        ModalCenterZoneColumn.Width = new GridLength(center, GridUnitType.Star);
        ModalNextZoneColumn.Width = new GridLength(normalized, GridUnitType.Star);
        ...
        int? selectionFallbackIndex = null)
    {
        ...
        try
        {
            ...
            return new FilterSnapshot(
                ...
                request?.RestrictToCurrentProjection == true,
                _sortBy,
                _randomSortSeed,
                albumActive,
    ...
    private static FilterResult ComputeFilterResult(FilterSnapshot snapshot, CancellationToken cancellationToken)
    {
        ...
        int? exactSingleRemovalIndex = null;
        if (snapshot.RestrictToCurrentProjection
            && !snapshot.ReorderCatalog
            && filteredCount == snapshot.TileCount - 1)
        {
            int removedIndex = 0;
            while (removedIndex < filteredCount
                && ReferenceEquals(snapshot.Tiles[removedIndex].Tile, filtered[removedIndex]))
            {
                removedIndex++;
            }

            bool exactRemoval = true;
            for (int index = removedIndex; index < filteredCount; index++)
            {
                if (ReferenceEquals(snapshot.Tiles[index + 1].Tile, filtered[index]))
                    continue;
                exactRemoval = false;
                break;
            }
    ...
    private void StageGalleryAutomationProjection(
        GalleryAutomationProjectionIndex projection)
    {
        try
        {
            CardsList.StageAutomationProjection(projection);
            RowsList.StageAutomationProjection(projection);
        }
        finally
        {
        ...
        long generation)
    {
        ...
        long preResetMs = 0;
        long preResetSelectionReleaseMs = 0;
        bool preResetSelectionCleared = true;
        int preResetSelectionReleasedCount = 0;
        long resetMs = 0;
        long selectionCommitMs = 0;
        double resetGeneratorRemoveMs = 0;
        ...
        // A focused selection is the viewport authority. Scheduling the old
        // viewport anchor and then immediately bringing the focused item into
        // view caused two competing render/layout passes after a Reset.
        if (filterResult.ViewportAnchor is not null && !restoreGalleryFocus)
    ...
    private void ApplyRightPanelPlacement(bool open)
    {
        if (_adaptiveWorkbench)
        {
            ...
            WorkbenchPreviewRow.Height = open ? new GridLength(CurrentAdaptivePreviewHeight()) : new GridLength(0);
            RightSplitterCol.Width = new GridLength(0);
            RightCol.MinWidth = 0;
            RightCol.MaxWidth = MaxRightPanelWidth;
            RightCol.Width = new GridLength(0);
            PreviewImageCanvas.Height = CurrentAdaptivePreviewImageHeight();
    ...
    private void ApplyAdaptiveWorkbenchLayout(double width, bool preserveGridAnchor = true)
    {
        ...
        {
            if (adaptive && RightPanel.Visibility == Visibility.Visible)
            {
                WorkbenchPreviewRow.Height = new GridLength(CurrentAdaptivePreviewHeight());
                PreviewImageCanvas.Height = CurrentAdaptivePreviewImageHeight();
            }
            return;
        }
    ...
    private double CurrentAdaptivePreviewHeight()
    {
        ...
        return Math.Clamp(
            Math.Round(availableHeight * AdaptivePreviewHeightRatio),
            AdaptivePreviewMinHeight,
            AdaptivePreviewMaxHeight);
    }

    private double CurrentAdaptivePreviewImageHeight()
        => Math.Clamp(
    ...
    private bool AdjustModalZoom(double multiplier, Point? anchor = null)
    {
    ...
    private bool SetModalZoom(double zoom, Point? anchor = null, bool animate = true)
    {
        ...
        double previous = _modalZoom;
        if (anchor.HasValue && ModalImage.ActualWidth > 0 && ModalImage.ActualHeight > 0)
        {
            Point point = anchor.Value;
            double offsetX = point.X - (ModalImage.ActualWidth / 2);
            double offsetY = point.Y - (ModalImage.ActualHeight / 2);
            double previousScaleX = _modalFlipped ? -previous : previous;
            double nextScaleX = _modalFlipped ? -next : next;
            _modalPanX += offsetX * (previousScaleX - nextScaleX);
            _modalPanY += offsetY * (previous - next);
        }

        _modalZoom = next;
        ClampModalPan();
        BeginModalTransformInteraction();
        UpdateModalTransform(animate);
        return true;
    }
    ...
    private void UpdateModalTransform(bool animate = false)
    {
        if (ModalVisualTransform is not null)
        {
            SetAnimatedTransformValue(
                ModalVisualTransform,
                ScaleTransform.ScaleXProperty,
                _modalFlipped ? -_modalZoom : _modalZoom,
                animate);
            SetAnimatedTransformValue(
                ModalVisualTransform,
        ...
        {
            SetAnimatedTransformValue(ModalPanTransform, TranslateTransform.XProperty, _modalPanX, animate);
            SetAnimatedTransformValue(ModalPanTransform, TranslateTransform.YProperty, _modalPanY, animate);
        }
    ...
    private void BeginModalTransformInteraction()
    {
        ...
        _modalTransformQualityTimer.Stop();
        _modalTransformQualityTimer.Start();
    }

    private void EndModalTransformInteraction()
    {
        _modalTransformQualityTimer.Stop();
        if (ModalBitmap is not null)
            RenderOptions.SetBitmapScalingMode(ModalBitmap, BitmapScalingMode.HighQuality);
    ...
    private void UpdateModalFit()
    {
        ...
        double availableWidth = Math.Max(1, areaWidth);
        double availableHeight = Math.Max(1, areaHeight);
    ...
    private void HideModalChromeFromImage()
    {
        if (Modal.Visibility != Visibility.Visible)
            return;
    ...
    {
        bool visible = ModalChromeEffectivelyVisible;
        bool animateReveal = visible && !_modalManualChromeVisible && _modalTransientChromeVisible;
        SetModalChromeElementVisibility(ModalTopBarSurface, visible, animateReveal);
        ...
        bool animateReveal)
    {
        ...
        element.BeginAnimation(
            OpacityProperty,
            new DoubleAnimation(0.2, 1, TimeSpan.FromMilliseconds(ModalChromeRevealAnimationMilliseconds))
            {
                EasingFunction = new QuadraticEase { EasingMode = EasingMode.EaseOut },
                FillBehavior = FillBehavior.Stop,
            },
    ...
    private void SaveKeyBindings_Click(object sender, RoutedEventArgs e)
    {
        ...
        var candidate = new Dictionary<ViewerKeyAction, KeyChord>(_draftKeyBindings);
        if (!TryPersistSharedKeyBindings(candidate, out string? sharedError))
        {
        ...
        _sharedKeyBindingsProtected = false;
        _sharedKeyBindingsError = null;
    ...
    private static bool TryPersistSharedKeyBindings(
        IReadOnlyDictionary<ViewerKeyAction, KeyChord> bindings,
        out string? error)
    {
        error = null;
        IReadOnlyDictionary<string, string> browserBindings;
        string path;
        try
        {
            browserBindings = KeyBindingSettings.ToBrowserSharedBindings(bindings);
            path = ThumbnailStatusBorderSettingsPath;
        }
        catch (Exception ex)
        {
            error = $"Shared settings path or shortcut mapping is unavailable: {ex.Message}";
            return false;
        }
        ...
        bool saved = TryWithPersistenceLock(path, () =>
        {
            ...
            if (!ThumbnailStatusBorderSettingsStore.TryMergeBrowserKeyBindings(
                    existingJson,
                    browserBindings,
            ...
            if (parsed.IsProtected
                || !KeyBindingSettings.TryApplyBrowserSharedBindings(
                    bindings,
                    parsed.BrowserKeyBindings,
                    out Dictionary<ViewerKeyAction, KeyChord> roundTripped,
                    out operationError))
            {
                operationError ??= parsed.Error ?? "The merged shared shortcut settings were invalid.";
                return false;
            }
            IReadOnlyDictionary<string, string> roundTrippedBrowser =
                KeyBindingSettings.ToBrowserSharedBindings(roundTripped);
            if (browserBindings.Any(pair =>
                    !roundTrippedBrowser.TryGetValue(pair.Key, out string? value)
                    || !string.Equals(value, pair.Value, StringComparison.Ordinal)))
            {
                operationError = "Shared shortcut round-trip did not preserve the requested bindings.";
                return false;
            }
            return TryWriteAtomicSharedJson(path, mergedJson, out operationError);
        });

        if (saved)
            return true;
        error = operationError ?? (operationStarted
            ? "The shared shortcut settings write failed."
    ...
    private void ApplyKeyBindingTooltips()
    {
        if (ModalCloseBtn is null)
            return;
        ModalCloseBtn.ToolTip = UiLanguageResources.Format("UiBackToGalleryFormat", BindingText(ViewerKeyAction.CloseModal));
        ModalPreviousButton.ToolTip = UiLanguageResources.Format("UiPreviousImageFormat", BindingText(ViewerKeyAction.PreviousImage));
        ...
        ModalFilmstripToggleButton.ToolTip = UiLanguageResources.Format(
            _modalFilmstripOpen ? "UiHideFilmstripFormat" : "UiShowFilmstripFormat",
            BindingText(ViewerKeyAction.ToggleModalFilmstrip));
        ModalShortcutHintText.Text = UiLanguageResources.Format(
            "UiModalShortcutFormat",
            BindingText(ViewerKeyAction.PreviousImage),
            BindingText(ViewerKeyAction.NextImage),
            BindingText(ViewerKeyAction.CloseModal));
    }
    ...
    private void LoadThumbnailStatusBorderSettings()
    {
        ...
        _sharedKeyBindingsProtected = loaded.IsProtected;
        _sharedKeyBindingsError = loaded.Error;
        if (!loaded.IsProtected)
        {
            if (KeyBindingSettings.TryApplyBrowserSharedBindings(
                    _keyBindings,
                    loaded.BrowserKeyBindings,
                    out Dictionary<ViewerKeyAction, KeyChord> sharedBindings,
                    out string? sharedBindingError))
            {
                _keyBindings = sharedBindings;
                _draftKeyBindings = new Dictionary<ViewerKeyAction, KeyChord>(_keyBindings);
                _sharedKeyBindingsProtected = false;
                _sharedKeyBindingsError = null;
            }
            ...
            {
                _sharedKeyBindingsProtected = true;
                _sharedKeyBindingsError = sharedBindingError;
            }
        }
    ...
    private void RefreshThumbnailStatusBorderIdleText()
    {
        SaveThumbnailBordersButton.Content = UiLanguageResources.Text(
            _thumbnailStatusBorderSettingsProtected ? "UiRetrySave" : "UiSaveBorders");
        ThumbnailBordersStatusText.Text = _thumbnailStatusBorderSettingsProtected
            ? UiLanguageResources.Format(
                "UiSharedSettingsProtectedFormat",
                _thumbnailStatusBorderSettingsError ?? "")
            : UiLanguageResources.Text("UiThumbnailBordersEditable");
    }
    ...
    private void ThumbnailBorderColor_TextChanged(object sender, TextChangedEventArgs e)
    {
        ...
        ThumbnailBordersStatusText.Text = valid
            ? UiLanguageResources.Text("UiBordersReadyToSave")
            : UiLanguageResources.Text("UiBorderHexHelp");
    }
    ...
    private void ThumbnailBorderEnabled_Changed(object sender, RoutedEventArgs e)
    {
        ...
        _dirtyThumbnailStatusBorderPreferences |= ReferenceEquals(sender, FavoriteThumbnailBorderCheckBox)
            ? ThumbnailStatusBorderDirtyPreferences.Favorite
            : ThumbnailStatusBorderDirtyPreferences.Enhanced;
        ThumbnailBordersStatusText.Text = UiLanguageResources.Text("UiBordersReadyToSave");
    }
    ...
    private void ResetThumbnailBorders_Click(object sender, RoutedEventArgs e)
    {
        ...
        PopulateThumbnailStatusBorderControls(_draftThumbnailStatusBorderSettings);
        SaveThumbnailBordersButton.Content = UiLanguageResources.Text("UiSaveBorders");
        EnhancedThumbnailBorderColorTextBox.Text = EnhancedStatusBorderDefaultColor;
        ThumbnailBordersStatusText.Text = UiLanguageResources.Text("UiBorderDefaultsReady");
    }
    ...
    private void SaveThumbnailBorders_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (_dirtyThumbnailStatusBorderPreferences == ThumbnailStatusBorderDirtyPreferences.None)
        {
            ...
            {
                SaveThumbnailBordersButton.Content = UiLanguageResources.Text("UiRetrySave");
                ThumbnailBordersStatusText.Text = UiLanguageResources.Text("UiProtectedChangeBeforeRetry");
            }
            else
            {
                _lastThumbnailStatusBorderSaveSucceeded = true;
                ThumbnailBordersStatusText.Text = UiLanguageResources.Text("UiNoBorderChanges");
            }
            return;
        }
        ...
        {
            SaveThumbnailBordersButton.Content = UiLanguageResources.Text("UiRetrySave");
            ThumbnailBordersStatusText.Text = UiLanguageResources.Text("UiBordersNeedHex");
            return;
        }
                ...
                out string? error))
        {
            ...
            SaveThumbnailBordersButton.Content = UiLanguageResources.Text("UiRetrySave");
            ThumbnailBordersStatusText.Text = UiLanguageResources.Format("UiCouldNotSaveSharedFormat", error ?? "");
            return;
        }
        ...
        _lastThumbnailStatusBorderSaveSucceeded = true;
        SaveThumbnailBordersButton.Content = UiLanguageResources.Text("UiSaveBorders");
        ThumbnailBordersStatusText.Text = UiLanguageResources.Text("UiBordersSaved");
    }
    ...
    private void SetTransientStatusToast(string status)
    {
        SetStatusToast(status);
        if (_statusRetryAction is not null || HasUnresolvedSharedFailures)
            return;
        _statusToastDismissTimer?.Stop();
        _statusToastDismissTimer?.Start();
    }
    ...
    public void ActivateAlbumPathsForSmoke(string albumId, IReadOnlyList<string> paths)
    {
        string now = DateTime.UtcNow.ToString("O", CultureInfo.InvariantCulture);
        IReadOnlyList<AlbumMemberEntry> members = paths
            .Select((path, index) => new AlbumMemberEntry($"member-{index}", Path.GetFullPath(path), now))
            .ToArray();
        ActivateAlbumSource(new AlbumEntry(albumId, "Prompt search Album", false, null, now, now, 1, members));
    }

    public async Task<bool> ActivateAlbumForSmokeAsync(string albumId)
    {
        AlbumReadResult read = await Task.Run(() => AlbumStore.Read(AlbumStore.ResolvePath()));
        AlbumEntry? album = read.Supported
            ? read.Document?.Albums.FirstOrDefault(candidate => candidate.Id == albumId)
        ...
        if (album is null)
            return false;
        await ActivateAlbumSourceAsync(album);
        return true;
    }
    ...
    public int ActiveAlbumAvailabilityStartedCountForSmoke => Volatile.Read(ref _activeAlbumAvailabilityStartedCount);
    public int ActiveAlbumAvailabilityCanceledCountForSmoke => Volatile.Read(ref _activeAlbumAvailabilityCanceledCount);
    public int ActiveAlbumAvailabilityMaxConcurrentCountForSmoke => Volatile.Read(ref _activeAlbumAvailabilityMaxConcurrentCount);
    public int ActiveAlbumAvailabilityActiveCountForSmoke => Volatile.Read(ref _activeAlbumAvailabilityActiveCount);
    public int ActiveAlbumAvailabilityPendingCountForSmoke => Volatile.Read(ref _activeAlbumAvailabilityPendingCount);
    public int ActiveAlbumAvailabilityApplyCountForSmoke => Volatile.Read(ref _activeAlbumAvailabilityApplyCount);
    public int ActiveAlbumSourceApplyCountForSmoke => Volatile.Read(ref _activeAlbumSourceApplyCount);
    public bool ActiveAlbumAvailabilityRanOffDispatcherForSmoke => _lastActiveAlbumAvailabilityOffDispatcher;

    public void ConfigureActiveAlbumAvailabilityDelayForSmoke(int milliseconds)
        => Volatile.Write(ref _activeAlbumAvailabilityDelayForSmokeMs, Math.Max(0, milliseconds));

    public async Task WaitForActiveAlbumAvailabilityStartedForSmokeAsync(int startedAfter, int timeoutMilliseconds = 5_000)
    {
        var timeout = Stopwatch.StartNew();
        while (Volatile.Read(ref _activeAlbumAvailabilityStartedCount) <= startedAfter)
        {
            if (timeout.ElapsedMilliseconds >= timeoutMilliseconds)
                throw new TimeoutException("Active Album availability scan did not start.");
            await Task.Delay(10);
        }
    }

    public async Task WaitForActiveAlbumAvailabilityIdleForSmokeAsync(int timeoutMilliseconds = 5_000)
    {
        var timeout = Stopwatch.StartNew();
        while (Volatile.Read(ref _activeAlbumAvailabilityPendingCount) > 0
            || Volatile.Read(ref _activeAlbumAvailabilityActiveCount) > 0)
        {
            if (timeout.ElapsedMilliseconds >= timeoutMilliseconds)
                throw new TimeoutException("Active Album availability scans did not stop.");
            await Task.Delay(10);
        }
    }
    ...
    public async Task<bool> AdjustGalleryFavoritePointerForSmokeAsync(string fileName, int delta)
    {
        ...
        SearchFilterCompletion? projection = _pendingSearchFilterCompletion is { } completion
            ? await completion.Task
            : null;
        return routeWasAllowedToUnwind
            && tile.Fav == expected
            && (projection is null || (projection.Value.Applied && !projection.Value.Discarded));
    }
    ...
    public long MaxEffectiveResidentThumbnailBudgetBytesForSmoke
        => Math.Max(_residentThumbnailBudgetBytes, _maxEffectiveResidentThumbnailBudgetBytes);
    public long MaxProtectedResidentThumbnailBytesForSmoke => _maxProtectedResidentThumbnailBytes;
    public int VisibleThumbnailEvictionCountForSmoke => _visibleThumbnailEvictionCount;
    ...
    public GalleryAutomationPeerSmokeLease? CreateGridAutomationPeerLeaseForSmoke(int index)
    {
        if (index < 0
            || index >= _tiles.Count
            ...
            || listPeer.GetOrCreateItemPeer(_tiles[index])
                is not VirtualizedGalleryListBoxItemAutomationPeer itemPeer)
        {
            return null;
        }
        return new GalleryAutomationPeerSmokeLease(itemPeer);
    }

    public int GridAutomationRealizeDispatchCountForSmoke
        => CardsList.AutomationRealizeDispatchCount;
    public int GridAutomationRealizeCoalescedCountForSmoke
        ...
        try
        {
            ...
            GalleryAutomationProjectionIndex automationProjection =
                GalleryAutomationProjectionIndex.Create(
                    ...
                    _allTiles.Count,
                    CancellationToken.None);
            StageGalleryAutomationProjection(automationProjection);
    ...
    public static async Task<PersistenceLockProbe> ProbePersistenceLockForSmokeAsync(string favoritesPath, string firstPath, string secondPath)
    {
        ...
        bool staleLockProtected = await Task.Run(() =>
        {
            ...
            string beforeDocument = File.ReadAllText(favoritesPath);
            bool protectedFile = !TryMergeFavoriteForSmoke(
                    favoritesPath,
                    Path.Combine(Path.GetDirectoryName(favoritesPath)!, "must-not-write-through-stale-lock.png"),
                    2)
                && string.Equals(File.ReadAllText(favoritesPath), beforeDocument, StringComparison.Ordinal)
                && File.Exists(lockPath)
                && string.Equals(File.ReadAllText(lockPath), stale, StringComparison.Ordinal);
            try { File.Delete(lockPath); } catch { }
        ...
        return new PersistenceLockProbe(concurrentMerged, staleLockProtected, malformedLockProtected);
    }
    ...
    internal static void CreateCrashedPersistenceWriterForSmoke(string targetPath, string readyPath)
    {
        ...
        // lock and pre-replace temp remain. The next writer must fail closed;
        // the TEMP-only verifier removes the proven orphan explicitly before
        // checking that normal residue cleanup and mutation can resume.
    ...
    public bool ModalZoomShortcutForSmoke(string shortcut)
    {
        ...
        return shortcut.ToLowerInvariant() switch
        {
            ...
            "zero" or "0" => ResetModalTransform(_modalTransformPath, showFeedback: true),
            _ => false,
        };
    }
    ...
    public bool ModalInteractiveScalingForSmoke
        => ModalBitmap is not null
            && RenderOptions.GetBitmapScalingMode(ModalBitmap) == BitmapScalingMode.Linear;
    public bool SettleModalTransformQualityForSmoke()
    {
        EndModalTransformInteraction();
        return ModalBitmap is not null
            && RenderOptions.GetBitmapScalingMode(ModalBitmap) == BitmapScalingMode.HighQuality;
    }
    ...
    public bool ModalWindowCaptionControlsContractForSmoke
        => Modal.Visibility == Visibility.Visible
            ...
            && ModalWindowCaptionControls.IsHitTestVisible
            && Panel.GetZIndex(ModalWindowCaptionControls) > Panel.GetZIndex(ModalTopBar)
            && WindowChrome.GetIsHitTestVisibleInChrome(ModalWindowCaptionControls)
            && new[] { ModalMinimizeButton, ModalMaximizeButton, ModalWindowCloseButton }.All(button =>
                button.Visibility == Visibility.Visible
                && button.IsEnabled
    ...
    public bool ActivateModalMaximizeForSmoke()
    {
    ...
    public bool ModalEdgeChromeContractForSmoke
        => ModalPreviousZoneColumn.Width.GridUnitType == GridUnitType.Star
            ...
            && ModalNextZoneColumn.Width.GridUnitType == GridUnitType.Star
            && Math.Abs(ModalPreviousZoneColumn.Width.Value - 5) < 0.01
            && Math.Abs(ModalCenterZoneColumn.Width.Value - 90) < 0.01
            && Math.Abs(ModalNextZoneColumn.Width.Value - 5) < 0.01
            && ModalPreviousButton.Content is System.Windows.Shapes.Path
            && ModalNextButton.Content is System.Windows.Shapes.Path
            && ReferenceEquals(ModalPreviousButton.Style, TryFindResource("ModalEdgeButton"))
    ...
    public bool ModalEdgeZonesAccessibleForSmoke
        => string.Equals(System.Windows.Automation.AutomationProperties.GetName(ModalPreviousButton), "Previous image edge zone", StringComparison.Ordinal)
            ...
            && ModalImageArea.ActualWidth > 0
            && Math.Abs((ModalPreviousZoneColumn.ActualWidth / ModalImageArea.ActualWidth) - 0.05) < 0.01
            && Math.Abs((ModalNextZoneColumn.ActualWidth / ModalImageArea.ActualWidth) - 0.05) < 0.01;
    public double ModalEdgeNavigationPercentForSmoke => _modalEdgeNavigationPercent;
    public bool SetModalEdgeNavigationPercentForSmoke(double value)
    {
        _ = SetModalEdgeNavigationPercent(value, persist: true);
        UpdateLayout();
        double expected = Math.Clamp(
            Math.Round(double.IsFinite(value) ? value : ModalEdgeNavigationDefaultPercent),
            ModalEdgeNavigationMinPercent,
            ModalEdgeNavigationMaxPercent);
        return Math.Abs(_modalEdgeNavigationPercent - expected) < 0.01
            && Math.Abs(ModalPreviousZoneColumn.Width.Value - expected) < 0.01
            && Math.Abs(ModalCenterZoneColumn.Width.Value - (100 - (expected * 2))) < 0.01
            && Math.Abs(ModalNextZoneColumn.Width.Value - expected) < 0.01
            && Math.Abs(ModalEdgeNavigationSlider.Value - expected) < 0.01;
    }
    public bool HideModalChromeFromImageForSmoke()
    {
    ...
    public async Task<bool> WaitForModalChromeTransientExpirationForSmokeAsync(int timeoutMilliseconds = 5000)
    {
        ...
        while (watch.ElapsedMilliseconds < timeoutMilliseconds)
        {
            if (!_modalManualChromeVisible && !_modalTransientChromeVisible && !ModalChromeEffectivelyVisible)
                return true;
            await Task.Delay(25);
        }
        return !_modalManualChromeVisible && !_modalTransientChromeVisible && !ModalChromeEffectivelyVisible;
    }
    ...
    {
        bool handled = InvokePreviewKeyForSmoke(Key.A, ModifierKeys.None);
        await WaitForModalEnhancementRequestForSmokeAsync();
        return handled && _modalEnhancementJobStatus is "queued" or "running";
    }
...
{
    public string Name => peer.GetName();

    public void Realize()
    {
        if (peer.GetPattern(PatternInterface.VirtualizedItem)
            is not IVirtualizedItemProvider provider)
        {
            throw new InvalidOperationException("VirtualizedItem pattern is not available.");
        }
        provider.Realize();
    }
    ...
    public void Select()
    {
        if (peer.GetPattern(PatternInterface.SelectionItem)
            is not ISelectionItemProvider provider)
        {
            throw new InvalidOperationException("SelectionItem pattern is not available.");
        }
        provider.Select();
    }
}
