...
確認日: 2026-09-16。実装基準: `3fe8d559d13edeffdc3214df410ce509b5b5f077`。
対象: ネイティブWPF版 Aibos Image、MiniMax H3向け動画スタイル、保存形式 Version 1。

この資料は、別のAIや編集者が会話履歴なしでスタイルを理解・編集するための、単独で読めるリファレンスです。例文は架空の通常動作です。既存スタイルの本文や個人の設定は含みません。新しい言語の提案ではなく、上記リビジョンの実装を説明します。将来の実装を保証するものではありません。
...
| `DirectionAspect` | `""` | 共通演技メニューとの対応。`""`, `opening`, `arms`, `expression`, `mood` |
...
| `ActionPlot` | `false` | AI強化時に選択時間内の最大3つの動作段階を考える |
...
| `ExpressionId` | `"original"` | 動画全体の表情。付録の登録IDのみ |
| `MoodId` | `"original"` | 主動作を行うときの全体の雰囲気。付録の登録IDのみ |
...
ActionPlotは選択時間内の始まり・展開・終わりを最大3段階で考える補助です。ActionSamplesは参考例であって、全部を強制追加するものではありません。強化OFF時はActionPlotとPhysicalContinuityの追加指示を出しません。
...
AI強化後の本文はジョブの実行用コピーです。保存済みスタイル、注釈前の本文、日本語説明、送信済み要求を上書きしません。強化が失敗したときに黙って未強化生成へ切り替える仕様でもありません。
...
| 表情 | 動画全体で主動作に反応する顔の方向性。顔を固定する指示ではない |
...
専用の `CameraId` やカメラ用JSONタイムラインはありません。カメラは本文冒頭の映像説明内にある `[]` の選択肢として記述し、Categoryをcameraまたはviewpointにします。
...
「低い角度から正面へ移り、その後顔へ寄る」のような順序は一つの候補内の時間順の文章で指定します。視点・注目先・角度を別トークンに分けることもできますが、矛盾する組合せをUIが自動で解消する保証はありません。最終本文を確認してください。
...
overall_soundscape: Quiet room ambience and the spoken line described above.
...
| 実行直前のAI強化契約 | [enhancement-video-prompt-enhancement-v1.json](../contracts/enhancement-video-prompt-enhancement-v1.json) |
