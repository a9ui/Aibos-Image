...
if ($ViewportChurnSeconds -gt 0 -and $result.viewportChurn.ok -ne $true) {
    $residentMiB = [Math]::Round($result.viewportChurn.maxResidentThumbnailBytes / 1MB, 1)
    $effectiveMiB = [Math]::Round($result.viewportChurn.maxEffectiveResidentThumbnailBudgetBytes / 1MB, 1)
    $structuralFailures += "viewport churn failed (selection loss $($result.viewportChurn.selectionLossCount), stale preview $($result.viewportChurn.stalePreviewCount), thumbnail timeout $($result.viewportChurn.thumbnailTimeoutCount), selection p95 $($result.viewportChurn.selectionP95Ms) ms, preview p95 $($result.viewportChurn.cachedPreviewP95Ms) ms, stalls $($result.viewportChurn.uiStallsOver250Ms), working-set regression $($result.viewportChurn.workingSetRegressionPercent)%, resident $residentMiB/$effectiveMiB MiB, visible evictions $($result.viewportChurn.visibleThumbnailEvictionCount), workers $($result.viewportChurn.maxActiveThumbnailWorkers)/$($result.viewportChurn.thumbnailWorkerLimit))"
}
