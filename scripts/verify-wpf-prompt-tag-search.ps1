...
if ($result.boundedMatchesExplicitlyOriginal -ne $true) { $failures += 'catalog prompt fallback chips did not explicitly identify their Original authority' }
if ($result.versionAuthoritiesRemainDistinct -ne $true) { $failures += 'displayed-version prompt tags and Original search matches were merged without preserving authority' }
