...
$process = Start-Process -FilePath 'dotnet' `
    -ArgumentList @(
        ('"{0}"' -f $dll),
        '--prompt-tag-search-smoke',
        ('"{0}"' -f $OutputPath)) `
    -WindowStyle Hidden -PassThru
if (-not $process.WaitForExit($OverallTimeoutSeconds * 1000)) {
    Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue
    throw "WPF prompt tag search smoke timed out after $OverallTimeoutSeconds seconds."
}
...
if (($result.initialTags -join '|') -ne 'studio portrait|soft light|1girl|fresh_tag:1.2') { $failures += 'prompt tags were not trimmed, deduped, and kept in stable order' }
...
if (($result.numericPrefixSuggestions -notcontains '1girl') -or $result.numericPrefixVisible -ne $true) { $failures += 'typing 1 did not expose the indexed 1girl suggestion' }
if ($result.enterCommittedSuggestion -ne $true) { $failures += 'Enter did not commit the selected suggestion as one search term' }
if (($result.softPrefixSuggestions -notcontains 'soft light') -or $result.clickCommittedSuggestion -ne $true) { $failures += 'click did not commit the prompt-derived suggestion as one search term' }
if ($result.backspaceRemovedWholeTerm -ne $true) { $failures += 'empty-input Backspace did not remove the last complete term' }
if ($result.coloredSearchTerms -ne $true) { $failures += 'active search terms did not use the stable color palette' }
if ($result.outsideClickDismissesPopup -ne $true -or $result.popupClosed -ne $true) { $failures += 'search suggestions/history popup did not retain outside-click dismissal' }
if ($result.boundedSearchMatchesVisible -ne $true) { $failures += 'search-matching prompt tokens omitted by segment, tag-count, or scan bounds were not visible in the modal' }
if ($result.samePromptHasNoOriginalOnlyMatches -ne $true) { $failures += 'search terms matched by the displayed prompt were misclassified as Original-only matches' }
if ($result.versionAuthoritiesRemainDistinct -ne $true) { $failures += 'displayed-version prompt tags and Original-only search matches were merged in the presentation model' }
if ($result.originalMatchesRenderedSeparately -ne $true) { $failures += 'Original-only search matches were not rendered in a separate accessible surface without prompt-mapping actions' }
if ($result.boundedPromptCopyCurrent -ne $true) { $failures += 'Copy Prompt did not use the displayed CurrentVersionTags collection, including bounded current matches' }
if ($result.originalOnlyExcludedFromCopy -ne $true) { $failures += 'Copy Prompt included an Original-only search match' }
