...
int Send(string target)
{
    ...
    foreach (string argument in new[] { "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", $"$ErrorActionPreference='Stop'; . '{helper}'; if (Send-AibosDesktopActivation -Identity 'smoke-{target}') {{ exit 0 }} else {{ exit 10 }}" })
